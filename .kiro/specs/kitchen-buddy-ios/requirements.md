...
7. Settings SHALL have a Health section with one switch per profile (all three on by default) and a Sources screen naming the data sources, the table version and food count, and the thresholds; every badge and the worksheet SHALL say the figures are estimates from typical ingredients, not medical advice.
...
11. A wrong match is a data problem, not a recipe problem: correcting a line SHALL, by default, map that ingredient name to the chosen food (or "don't count") in every recipe, now and later, as an append-only book-wide mapping; "Only this recipe" keeps it to one recipe (10). The picker SHALL lead with close matches (every food whose keyword appears in the name, best first, then foods sharing a word) before the full table, SHALL offer "Report this match" that pre-fills feedback with the ingredient, the automatic match and the table version, and Settings › Health SHALL list the mappings in force with a way back to automatic. Mappings travel in the export file (2.2). *(Added 2026-09-10 from beta feedback.)*
