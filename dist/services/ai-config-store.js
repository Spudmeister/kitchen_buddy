...
export class AIConfigStore {
    ...
    async saveConfig(config) {
        ...
        this.db.transaction(() => {
            ...
            // Store provider
            this.db.run('INSERT INTO ai_config (key, value) VALUES (?, ?)', [CONFIG_KEYS.PROVIDER, config.provider]);
            // Store API key (encrypted)
            if (config.apiKey) {
                const encryptedKey = this.encrypt(config.apiKey, salt);
                this.db.run('INSERT INTO ai_config (key, value) VALUES (?, ?)', [CONFIG_KEYS.API_KEY, encryptedKey]);
            }
            // Store endpoint
            if (config.endpoint) {
                this.db.run('INSERT INTO ai_config (key, value) VALUES (?, ?)', [CONFIG_KEYS.ENDPOINT, config.endpoint]);
            }
            // Store model
            if (config.model) {
                this.db.run('INSERT INTO ai_config (key, value) VALUES (?, ?)', [CONFIG_KEYS.MODEL, config.model]);
            }
            // Store salt
            this.db.run('INSERT INTO ai_config (key, value) VALUES (?, ?)', [CONFIG_KEYS.ENCRYPTION_SALT, salt]);
        });
    }
    /**
    ...
    async loadConfig() {
        // Get provider
        const providerRow = this.db.get('SELECT value FROM ai_config WHERE key = ?', [CONFIG_KEYS.PROVIDER]);
        if (!providerRow) {
            return null;
        }
        const provider = providerRow[0];
        // Get salt for decryption
        const saltRow = this.db.get('SELECT value FROM ai_config WHERE key = ?', [CONFIG_KEYS.ENCRYPTION_SALT]);
        const salt = saltRow?.[0] || this.getOrCreateSalt();
        // Get encrypted API key
        const apiKeyRow = this.db.get('SELECT value FROM ai_config WHERE key = ?', [CONFIG_KEYS.API_KEY]);
        let apiKey;
        if (apiKeyRow) {
            try {
        ...
        // Get endpoint
        const endpointRow = this.db.get('SELECT value FROM ai_config WHERE key = ?', [CONFIG_KEYS.ENDPOINT]);
        // Get model
        const modelRow = this.db.get('SELECT value FROM ai_config WHERE key = ?', [CONFIG_KEYS.MODEL]);
        return {
            provider,
            apiKey,
     ...
     */
    isConfigured() {
        const row = this.db.get('SELECT COUNT(*) FROM ai_config WHERE key = ?', [CONFIG_KEYS.PROVIDER]);
        return (row?.[0] ?? 0) > 0;
    }
    /**
     * Get or create encryption salt
     */
    getOrCreateSalt() {
        const existingRow = this.db.get('SELECT value FROM ai_config WHERE key = ?', [CONFIG_KEYS.ENCRYPTION_SALT]);
        if (existingRow) {
...
//# sourceMappingURL=ai-config-store.js.map