...
export class ExportService {
     ...
     */
    exportFolder(folderId, options = {}) {
        // Get folder info
        const folderRow = this.db.get('SELECT id, name, parent_id, created_at FROM folders WHERE id = ?', [folderId]);
        if (!folderRow) {
        ...
        const recipeRows = this.db.exec('SELECT id FROM recipes WHERE folder_id = ? AND archived_at IS NULL', [folderId]);
    ...
    exportAll(options = {}) {
        ...
        const exportedRecipes = recipeIds
            ...
            .map(r => this.recipeToExported(r));
        // Get all folders
        const folderRows = this.db.exec('SELECT id, name, parent_id, created_at FROM folders');
    ...
    importRecipes(data) {
        ...
        for (let i = 0; i < data.recipes.length; i++) {
            ...
            try {
                ...
                result.errors.push(`Recipe ${i} (${exportedRecipe.title}): ${error.message}`);
            }
        }
        ...
        if (data.cookSessions) {
            for (const session of data.cookSessions) {
                try {
                    ...
                    result.errors.push(`Cook session ${session.id}: ${error.message}`);
                }
            }
        }
        // Import ratings
        if (data.ratings) {
            for (const rating of data.ratings) {
                try {
                    ...
                    result.errors.push(`Rating ${rating.id}: ${error.message}`);
                }
            }
        }
        // Import photos
     ...
     */
    getCookSessionsForRecipes(recipeIds) {
        const sessions = [];
        for (const recipeId of recipeIds) {
     ...
     */
    getRatingsForRecipes(recipeIds) {
        const ratings = [];
        for (const recipeId of recipeIds) {
     ...
     */
    getInstancesForRecipes(recipeIds) {
        const instances = [];
        for (const recipeId of recipeIds) {
     ...
     * Import folders
     */
    importFolders(folders, result) {
        for (const folder of folders) {
            try {
                // Check if folder already exists
                const existing = this.db.get('SELECT id FROM folders WHERE id = ?', [folder.id]);
                if (!existing) {
                    this.db.run('INSERT INTO folders (id, name, parent_id, created_at) VALUES (?, ?, ?, ?)', [folder.id, folder.name, folder.parentId ?? null, folder.createdAt]);
                }
            }
            catch (error) {
     ...
     */
    importSingleRecipe(exported, recipeIdMap) {
        ...
        return this.db.transaction(() => {
         ...
         VALUES (?, 1, ?, ?, ?)`, [newRecipeId, null, parentRecipeId, exported.createdAt || now]);
           ...
           VALUES (?, ?, ?, ?, ?, ?)`, [
            ...
            if (exported.tags && exported.tags.length > 0) {
                for (const tagName of exported.tags) {
                    ...
                    this.db.run('INSERT OR IGNORE INTO recipe_tags (recipe_id, tag_id) VALUES (?, ?)', [newRecipeId, tagId]);
                }
            }
            // Import rating if present
            if (exported.rating !== undefined && exported.rating >= 1 && exported.rating <= 5) {
                const ratingId = uuidv4();
                this.db.run('INSERT INTO ratings (id, recipe_id, rating, rated_at) VALUES (?, ?, ?, ?)', [ratingId, newRecipeId, exported.rating, now]);
            }
            return newRecipeId;
        });
    }
     ...
     */
    importCookSession(session, newRecipeId) {
     ...
     * Import a rating
     */
    importRating(rating, newRecipeId) {
        const ratingId = uuidv4();
        this.db.run('INSERT INTO ratings (id, recipe_id, rating, rated_at) VALUES (?, ?, ?, ?)', [ratingId, newRecipeId, rating.rating, rating.ratedAt]);
    }
    /**
     * Import a recipe instance
     */
    importInstance(instance, newRecipeId) {
    ...
    getAllRecipes() {
        const recipeRows = this.db.exec('SELECT id FROM recipes WHERE archived_at IS NULL');
        return recipeRows
            .map(row => this.recipeService.getRecipe(row[0]))
            .filter((r) => r !== undefined);
    }
    /**
     * Create a folder
    ...
    createFolder(name, parentId) {
        ...
        const now = new Date().toISOString();
        this.db.run('INSERT INTO folders (id, name, parent_id, created_at) VALUES (?, ?, ?, ?)', [folderId, name, parentId ?? null, now]);
        return {
            id: folderId,
            name,
            parentId,
    ...
    moveToFolder(recipeId, folderId) {
        const result = this.db.run('UPDATE recipes SET folder_id = ? WHERE id = ?', [folderId, recipeId]);
        if (result.changes === 0) {
            throw new Error(`Recipe not found: ${recipeId}`);
        }
    }
     ...
     */
    getFolder(folderId) {
        const row = this.db.get('SELECT id, name, parent_id, created_at FROM folders WHERE id = ?', [folderId]);
        if (!row) {
            return undefined;
        }
        return {
