...
export class MenuAssistantService {
     ...
     */
    setVarietyPreferences(preferences) {
     ...
     */
    filterRecipesByConstraints(limit = 10) {
     ...
     * @returns Filtered recipes matching all constraints
     */
    filterRecipes(constraints, limit = 10) {
        const filters = {};
        ...
        if (constraints.excludeIngredients && constraints.excludeIngredients.length > 0) {
            ...
            recipes = recipes.filter(recipe => {
                return !recipe.ingredients.some(ing => excludeLower.some(excl => ing.name.toLowerCase().includes(excl)));
            });
        }
     ...
     */
    acceptSuggestion(recipeId, date, mealSlot, servings) {
     ...
     */
    acceptMultipleSuggestions(suggestions) {
        return suggestions.map(suggestion => this.acceptSuggestion(suggestion.recipeId, suggestion.date, suggestion.mealSlot, suggestion.servings));
    }
    /**
     ...
     */
    acceptSuggestionByIndex(suggestionIndex, date, mealSlot, servings) {
        ...
        return this.acceptSuggestion(suggestion.recipe.id, date, mealSlot, servings);
    }
    /**
     ...
     */
    createAndSetMenu(name, startDate, endDate) {
     ...
     */
    async getAlternatives(excludeRecipeIds) {
        const filters = this.buildFiltersFromConstraints();
        ...
        const filtered = recipes.filter(r => !excludeRecipeIds.includes(r.id) &&
     ...
     */
    calculateVarietyConfidence(recipe) {
        const usedCuisines = new Set(this.getUsedCuisines());
        const usedIngredients = new Set(this.getUsedMainIngredients());
        let confidence = 0.5;
     ...
     */
    async getSuggestionsForIntent(intent, message) {
        const filters = this.buildFiltersFromConstraints();
        let recipes = [];
        switch (intent) {
    ...
    generateSuggestionReason(recipe) {
        ...
        // Check dietary tags
        const dietaryTags = recipe.tags.filter(t => ['vegetarian', 'vegan', 'gluten-free', 'dairy-free', 'healthy'].includes(t.toLowerCase()));
        if (dietaryTags.length > 0) {
     ...
     */
    calculateConfidence(recipe, intent) {
        let confidence = 0.5;
        // Boost for rating
        if (recipe.rating) {
        ...
        if (intent === 'request_dietary') {
            const hasDietaryTags = recipe.tags.some(t => ['vegetarian', 'vegan', 'gluten-free', 'dairy-free', 'healthy'].includes(t.toLowerCase()));
            if (hasDietaryTags) {
                confidence += 0.2;
            }
        }
     ...
     * Build response using AI
     */
    async buildResponse(userMessage, suggestions, intent) {
     ...
     */
    formatAssistantMessage(aiResponse, suggestions) {
     ...
     */
    buildFallbackResponse(suggestions, intent) {
        let message;
        if (suggestions.length === 0) {
     ...
     */
    getAllRecipes() {
        const rows = this.db.exec('SELECT id FROM recipes WHERE archived_at IS NULL LIMIT 100');
        return rows
            .map(row => this.recipeService.getRecipe(row[0]))
            .filter((r) => r !== undefined);
    }
}
//# sourceMappingURL=menu-assistant-service.js.map