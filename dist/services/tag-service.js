...
export class TagService {
    ...
    addTag(recipeId, tagName) {
        ...
        this.db.transaction(() => {
            // Get or create tag
            let tagId;
            const existingTag = this.db.get('SELECT id FROM tags WHERE name = ?', [normalizedTag]);
            if (existingTag) {
                tagId = existingTag[0];
            }
            else {
     ...
     */
    removeTag(recipeId, tagName) {
        const normalizedTag = tagName.trim().toLowerCase();
        // Get tag ID
        const tagRow = this.db.get('SELECT id FROM tags WHERE name = ?', [normalizedTag]);
        if (!tagRow) {
    ...
    getRecipesByTag(tagName) {
        ...
        const recipeRows = this.db.exec(`SELECT r.id FROM recipes r
       JOIN recipe_tags rt ON r.id = rt.recipe_id
       JOIN tags t ON rt.tag_id = t.id
       WHERE t.name = ? AND r.archived_at IS NULL`, [normalizedTag]);
        // Load full recipe data for each
        return recipeRows
            .map((row) => this.loadRecipe(row[0]))
     ...
     */
    getAllTags() {
        const rows = this.db.exec(`SELECT t.name, COUNT(rt.recipe_id) as count
       FROM tags t
     ...
     */
    hasTag(recipeId, tagName) {
        const normalizedTag = tagName.trim().toLowerCase();
        const row = this.db.get(`SELECT 1 FROM recipe_tags rt
       JOIN tags t ON rt.tag_id = t.id
       WHERE rt.recipe_id = ? AND t.name = ?`, [recipeId, normalizedTag]);
     ...
     */
    cleanupOrphanedTags() {
        this.db.run(`DELETE FROM tags WHERE id NOT IN (
     ...
     */
    containsAny(ingredientNames, keywords) {
...
export class AITagService extends TagService {
     ...
     */
    applySuggestedTags(recipeId, suggestions, minConfidence = 0.7) {
