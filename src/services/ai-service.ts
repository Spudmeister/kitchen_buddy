...
export class AIService {
  private configStore: AIConfigStore;
   ...
   */
  private validateConfig(config: AIConfig): string | null {
    // Check provider
   ...
   */
  private isConfigValid(config: AIConfig): boolean {
   ...
   */
  private async sendCustomRequest(prompt: string): Promise<string> {
    const endpoint = this.getEndpoint();
    const model = this.getModel();
   ...
   */
  private buildRecipeParsePrompt(html: string): string {
  ...
  private parseRecipeResponse(response: string): ParsedRecipe {
    try {
      ...
      return {
        ...
        prepTime: data.prepTime ? this.parseISODuration(data.prepTime as string) : undefined,
        cookTime: data.cookTime ? this.parseISODuration(data.cookTime as string) : undefined,
...
Only return the JSON array, no other text.`;
   ...
   */
  private parseTagSuggestionResponse(response: string): TagSuggestion[] {
    try {
      ...
      return data
        .filter((item): item is Record<string, unknown> => 
          typeof item === 'object' && item !== null && 'tag' in item && 'confidence' in item
        )
   ...
   */
  private buildSubstitutionPrompt(ingredientName: string): string {
...
Consider common dietary restrictions and availability. Only return the JSON array, no other text.`;
   ...
   */
  private parseSubstitutionResponse(response: string, original: string): Substitution[] {
    try {
      const jsonMatch = response.match(/\[[\s\S]*\]/);
      if (!jsonMatch) {
        return [];
      ...
      return data
        .filter((item): item is Record<string, unknown> => 
          typeof item === 'object' && item !== null && 'substitute' in item
        )
