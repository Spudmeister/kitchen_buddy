 ...
 */
export function extractSchemaOrg(html: string): SchemaOrgRecipe | null {
 ...
 */
function extractJsonLd(html: string): SchemaOrgRecipe | null {
 ...
 */
function extractMicrodata(html: string): SchemaOrgRecipe | null {
  // Find the recipe container with itemtype="https://schema.org/Recipe"
  const recipeMatch = html.match(
    /<[^>]+itemtype=["']https?:\/\/schema\.org\/Recipe["'][^>]*>([\s\S]*?)(?=<[^>]+itemtype=["']https?:\/\/schema\.org\/|$)/i
  );
  ...
  const recipe: SchemaOrgRecipe = { '@type': 'Recipe' };

  // Extract name
  const nameMatch = recipeHtml.match(
  ...
  // Extract description
  const descMatch = recipeHtml.match(
    /<[^>]+itemprop=["']description["'][^>]*>([^<]*)<|itemprop=["']description["'][^>]*content=["']([^"']+)["']/i
  );
  if (descMatch) {
  ...
  const ingredientRegex =
    /<[^>]+itemprop=["']recipeIngredient["'][^>]*>([^<]*)<|itemprop=["']recipeIngredient["'][^>]*content=["']([^"']+)["']/gi;
  ...
  const instructionRegex =
    /<[^>]+itemprop=["']recipeInstructions["'][^>]*>([^<]*)<|itemprop=["']recipeInstructions["'][^>]*content=["']([^"']+)["']/gi;
  ...
  // Extract prep time
  const prepTimeMatch = recipeHtml.match(
    /itemprop=["']prepTime["'][^>]*content=["']([^"']+)["']|itemprop=["']prepTime["'][^>]*datetime=["']([^"']+)["']/i
  );
  if (prepTimeMatch) {
  ...
  // Extract cook time
  const cookTimeMatch = recipeHtml.match(
    /itemprop=["']cookTime["'][^>]*content=["']([^"']+)["']|itemprop=["']cookTime["'][^>]*datetime=["']([^"']+)["']/i
  );
  if (cookTimeMatch) {
  ...
  const yieldMatch = recipeHtml.match(
    /<[^>]+itemprop=["']recipeYield["'][^>]*>([^<]*)<|itemprop=["']recipeYield["'][^>]*content=["']([^"']+)["']/i
  );
  if (yieldMatch) {
  ...
  // Extract image
  const imageMatch = recipeHtml.match(
 ...
 */
export function schemaOrgToRecipe(schema: SchemaOrgRecipe, sourceUrl?: string): ParsedRecipe {
...
function extractInstructions(
  instructions: SchemaOrgInstruction[] | string[] | string | undefined
): string[] {
  if (!instructions) {
    return [];
  }
  ...
  if (Array.isArray(instructions) && instructions.length > 0) {
    if (typeof instructions[0] === 'string') {
      return (instructions as string[]).map((s) => s.trim()).filter(Boolean);
    }

    // Array of HowToStep/HowToSection objects
    return flattenInstructionObjects(instructions as SchemaOrgInstruction[]);
  }
 ...
 */
function flattenInstructionObjects(instructions: SchemaOrgInstruction[]): string[] {
  const result: string[] = [];
...
function extractImageUrl(
  image: string | { url?: string } | string[] | { url?: string }[] | undefined
): string | undefined {
  if (!image) {
    return undefined;
  }
 ...
 */
export function parseFromHtml(html: string, sourceUrl?: string): ParseResult {
  ...
  if (recipe.ingredients.length > 0) confidence += 0.15;
  if (recipe.instructions.length > 0) confidence += 0.15;
  if (recipe.prepTime || recipe.cookTime) confidence += 0.05;
  if (recipe.servings) confidence += 0.05;
 ...
 */
function extractUnit(text: string): { unit: Unit; remaining: string } {
  const trimmed = text.trim().toLowerCase();

  // Sort aliases by length (longest first) to match "fluid ounce" before "ounce"
  const sortedAliases = Object.entries(UNIT_ALIASES).sort(
 ...
 */
function findOriginalLength(original: string, alias: string): number {
  ...
  if (idx === 0) {
    ...
    while (end < original.length && (original[end] === '.' || original[end] === ' ')) {
      ...
      if (original[end - 1] === '.') break; // Only skip one period
    }
    return end;
  }
  return alias.length;
 ...
 */
function extractNameAndNotes(text: string): { name: string; notes?: string } {
 ...
 */
export function normalizeIngredients(rawIngredients: string[]): NormalizedIngredient[] {
 ...
 */
export async function parseFromHtmlWithAI(
  html: string,
  sourceUrl?: string,
  ...
  if (options?.useAI && options?.aiService?.isEnabled()) {
    try {
      ...
      if (aiRecipe.title && aiRecipe.title !== 'Untitled Recipe') confidence += 0.1;
      if (aiRecipe.ingredients.length > 0) confidence += 0.1;
      if (aiRecipe.instructions.length > 0) confidence += 0.1;
      if (aiRecipe.prepTime || aiRecipe.cookTime) confidence += 0.05;
      if (aiRecipe.servings) confidence += 0.05;
 ...
 * @returns Parse result with recipe data
 */
export async function parseFromUrlWithAI(
  url: string,
  options: ExtendedParseOptions = {}
): Promise<ParseResult> {
  const opts = { ...DEFAULT_PARSE_OPTIONS, ...options };
