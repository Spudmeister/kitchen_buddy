...
export class RecommendationService {
   ...
   * Requirements: 15.1
   */
  getRecommendations(filters: RecommendationFilters, limit: number = 10): Recipe[] {
    let sql = `
    ...
    const conditions: string[] = ['r.archived_at IS NULL'];
    ...
    let ids = rows.map(row => row[0] as string);
   ...
   */
  private filterByIngredients(recipeIds: string[], availableIngredients: string[]): string[] {
    const normalizedAvailable = availableIngredients.map(i => i.toLowerCase().trim());

    const recipesWithMissing: Array<{ id: string; missing: number }> = [];
    ...
    for (const id of recipeIds) {
      ...
      for (const ingredient of recipe.ingredients) {
        ...
        const hasIngredient = normalizedAvailable.some(avail =>
          ingredientName.includes(avail) || avail.includes(ingredientName)
        );
        if (!hasIngredient) {
          missingCount++;
        }
