...
export class StatisticsService {
  ...
  rateRecipe(input: RatingInput): RatingEntry {
    ...
    this.db.run(
      `INSERT INTO ratings (id, recipe_id, rating, rated_at)
       VALUES (?, ?, ?, ?)`,
      [id, input.recipeId, input.rating, now]
    );
  ...
  getAllRecipeRatings(): Map<string, number> {
    // Use rowid as tiebreaker when timestamps are the same
    const rows = this.db.exec(
      `SELECT recipe_id, rating FROM ratings r1
  ...
  getRecipeIdsByMinRating(minRating: number): string[] {
    // Use rowid as tiebreaker when timestamps are the same
    const rows = this.db.exec(
      `SELECT DISTINCT recipe_id FROM ratings r1
       WHERE rating >= ?
       AND rowid = (SELECT rowid FROM ratings r2 WHERE r2.recipe_id = r1.recipe_id ORDER BY rated_at DESC, rowid DESC LIMIT 1)`,
      [minRating]
    );
   ...
   */
  getRecipeIdsSortedByRating(order: 'asc' | 'desc' = 'desc'): string[] {
