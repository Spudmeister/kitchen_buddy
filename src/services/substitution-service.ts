...
export class SubstitutionService {
  ...
  suggestRecipesByIngredients(
    ...
    for (const row of recipeRows) {
      ...
      for (const ingredient of recipe.ingredients) {
        ...
        const isMatched = normalizedAvailable.some(avail => 
          this.ingredientsMatch(normalizedIngredient, avail)
        );
   ...
   */
  private ingredientsMatch(ingredient: string, available: string): boolean {
    // Exact match
